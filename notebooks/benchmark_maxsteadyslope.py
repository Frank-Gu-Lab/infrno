# -*- coding: utf-8 -*-
"""
Benchmark modelling pipelines using only Disco Effect maximum, steady state, and linear region slope.
"""
import pandas as pd
import numpy as np
...
df['molecular_weight'] = df['polymer_name'].apply(
    lambda x: extract_molecular_weight(x))
zipcode_df, ppm_bin_conversion_dict, polymer_name_conversion_dict = generate_polymer_zip_codes(
    df, kind='cohort')
interim_modelling_data = df.copy().drop(columns=[
    'amp_factor', 'AFo', 'SSE', 'alpha', 'beta', 'sample_size', 'proton_peak_index'])
interim_modelling_data = append_multilabel_zipcodes(
    interim_modelling_data, ppm_bin_conversion_dict)
interim_modelling_data = interim_modelling_data.loc[:, (
    interim_modelling_data != 0).any(axis=0)]
modelling_df = interim_modelling_data.copy().drop(
    columns=["polymer_zip_code", "concentration", 'ppm_bin', "polymer_name", "ppm_bin_codes", "polymer_name_codes"])
...
# compute slope, max prox, steady state prox of disco effect as features
def grab_max_prox(row_data):
  '''Reads row of a dataframe, grabs maximum disco effect from the saturation times passed.'''

  disco_effect = row_data[['corr_%_attenuation0.25',  'corr_%_attenuation0.5', 'corr_%_attenuation0.75',  'corr_%_attenuation1.0',
                           'corr_%_attenuation1.25',  'corr_%_attenuation1.5',
                           'corr_%_attenuation1.75']]

  max_prox = np.max(disco_effect)

  return max_prox

steady_state_proximity = modelling_df['corr_%_attenuation1.75']
max_proximity = [grab_max_prox(row) for _, row in modelling_df.iterrows()]
rateofchange = modelling_df['corr_%_attenuation1.0'] - modelling_df['corr_%_attenuation0.25']

modelling_df = modelling_df.drop(columns = ['corr_%_attenuation0.25',  'corr_%_attenuation0.5',
       'corr_%_attenuation0.75',  'corr_%_attenuation1.0',
       'corr_%_attenuation1.25',  'corr_%_attenuation1.5',
       'corr_%_attenuation1.75'])

modelling_df.insert(loc=0, column='steady_state_proximity', value=steady_state_proximity)
modelling_df.insert(loc=1, column='max_proximity', value=max_proximity)
modelling_df.insert(loc=2, column='linear_proximity_change_rate', value=rateofchange)
...
f1_scores.to_csv("../data/processed/max_slope_steady_f1.csv")
best_fully_trained_params.to_csv("../data/processed/maxsteadyslope_bestparams.csv")
